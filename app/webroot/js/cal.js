...
calObj[0].month["7-1-3"] = 'Holyda'; // 7月 月曜日(1) 第3 ということね。
...
calObj[0].clickURI = '_YEAR_,_MONTH_,_DAY_';
...
// 優先度 クラス指定する順番が変わります。
calObj[0].priority = new Array('week','xDay','xDaysLater','day','backward','forward');
...
function cal_create(calId){
	...
	for(dayCnt=1;dayCnt<=(calObj.days[month]+bisDay);dayCnt++){
		...
		// forward
		if(calObj[calId].forward != null && currentDayDate.getTime() > calObj.date.getTime())
			dayClass["forward"] = calObj[calId].forward;
...
function cal_open(uri){
	...
	for (var i = 1; i <= date.length - 1; i++) {
		date[i] = (date[i].length < 2)? "0" + date[i] : date[i] ;
	};
	// $.ajax({
	// 	type: 'POST',
	// 	url: '/users/getData',
    ...
    window.open("/users/mypage/" + element.dataset.id + "?date=" + date[0] + date[1] + date[2]);
}
